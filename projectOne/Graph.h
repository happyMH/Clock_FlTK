...
struct Line : Shape {            // a Line is a Shape defined by two Points
	...
	Line() {}
public :
	void set_place(Point p1, Point p2);
	void clear();
};
...
struct Text : Shape {
	...
	Text(const string s = " ") : lab(s), fnt(fl_font()), fnt_sz(fl_size()) {}

    void draw_lines() const;
	void set_place(Point x) { add(x); }
...
struct Circle : Shape {
	...
	void set_center(Point p) { add(Point(p.x - r, p.y - r)); }
private:
    int r;
};
